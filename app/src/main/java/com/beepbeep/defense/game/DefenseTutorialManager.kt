...
class DefenseTutorialManager(
    ...
    private suspend fun runStep1() {
        ...
        delay(10000)
        ttsManager.speakAndWait("사용하는 버튼 위치를 알려드립니다. " +
                "컨트롤러를 양손으로 쥐었을 때, 오른쪽 버튼 뭉치중 아래에 있는 버튼이 A 버튼입니다. 오른쪽에 있는 버튼이 B 버튼입니다." +
                " 왼쪽에 있는 버튼이 X 버튼입니다. " +
                "컨트롤러 왼쪽 위 모서리에서 왼손 검지로 누르는 버튼중 가장 위에 있는 버튼이 LB 버튼이고," +
                "오른쪽 위 모서리에서 오른손 검지로 누르는 버튼중 가장 위에 있는 버튼이 RB 버튼입니다.")
        delay(300)
    ...
    private suspend fun runStep3() {
        ...
        ttsManager.speakAndWait(
            "캐릭터와 공의 거리가 3미터 이내일 때 A 버튼을 눌러 포구하면 성공입니다, 3미터 밖에서 A버튼을 누르면 포구 실패입니다." +
                    ...
                    "포구 버튼 이후 다음 공은 X 버튼을 눌러야 다음 공이 날아옵니다."
        )
    ...
    private suspend fun finishTutorial() {
        ...
        ttsManager.speakAndWait(
            "튜토리얼이 완료되었습니다. " +
                    ...
                    "수비 훈련으로 다시 입장하세요. " +
                    "실제 수비 훈련 도중 뒤로가기나 강제 종료하면 조기종료로 판단하여 해당 진행중이던 기록이 저장됩니다."
        )
        delay(300)
        ttsManager.speakAndWait(
